...
if [ ! -f "../models/${MODEL_NAME}.onnx" ]; then
    echo "Downloading Voice Model (${MODEL_NAME})..."
    mkdir -p ../models
    wget -O ../models/${MODEL_NAME}.onnx https://huggingface.co/rhasspy/piper-voices/resolve/v1.0.0/es/es_ES/davefx/medium/${MODEL_NAME}.onnx
    wget -O ../models/${MODEL_NAME}.onnx.json https://huggingface.co/rhasspy/piper-voices/resolve/v1.0.0/es/es_ES/davefx/medium/${MODEL_NAME}.onnx.json
